...
def query_vs_target(model, dataset, args):
    ...
    for video in tqdm(loader):
        video_features = video[0][0]
        video_id = video[2][0]
        # print('video_id=', video_id, 'type(video_id)=', type(video_id))
        if video_id:  #  some video_id in the generated batch is '', so we must filter them
            features = model.index_video(video_features.to(args.gpu_id))
    ...
    # with open('my_FIVR_5k_res_json2.json', 'w') as f:
    #     import json
...
def queries_vs_database(model, dataset, args):
    ...
    for video in tqdm(loader):
        ...
        video_features = video[0][video_id != '']  # some video_id in the generated batch is '', so we must filter them
    ...
    for video in tqdm(loader):
        video_id = np.array(video[2])
        # print('0 video_id = ', video_id)
        video_features = video[0][video_id != '']
        video_mask = video[1][video_id != '']
        video_id = video_id[video_id != '']

        # if len(targets) >= 8: #todo
        #     break
    ...
    print('\n> Evaluation on {}'.format(dataset.name))
    # with open('./cg_sim_from_cg_student_bs1.json', 'w') as f:
    #     import json
    #     f.write(json.dumps(similarities))
    return dataset.evaluate(similarities, all_db)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='VCSL', choices=["FIVR-200K", "FIVR-5K", "CC_WEB_VIDEO", "SVD", "EVVE", 'VCSL'],
                        help='Name of evaluation dataset.')
    # parser.add_argument('--dataset_hdf5', type=str, default='/Users/zilliz/Downloads/fivr_5k.hdf5',
    parser.add_argument('--dataset_hdf5', type=str, default='./features/vcsl_feature.hdf5',
    ...
    parser.add_argument('--student_type', type=str, default='coarse-grained', choices=['fine-grained', 'coarse-grained'],
    ...
    parser.add_argument('--batch_sz', type=int, default=1,
                        help='Number of videos processed in each batch. Aplicable only with Coarse-greained Students.(There exists a issue: different batchsize will lead to different results, so you should make sure batchsize==1)')
    ...
    # with open('youtube_ids_5k.txt', 'w') as f:
    #     for yt_id in dataset.database:
    #         f.write(yt_id + '\n')
    # with open('youtube_ids_5k_query.txt', 'w') as f:
    #     for yt_id in dataset.queries:
    #         f.write(yt_id + '\n')
