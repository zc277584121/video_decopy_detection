...
def sim_map_gpu(qid, rid, q: np.ndarray, r: np.ndarray, normalize_input=False, similarity_type="cos", device='cuda:0') -> Tuple[str, str, np.ndarray]:
    """
    ...
    if len(q.shape) == 3:  # query_tenor.shape is (T, h*w, D), we must pool h*w before calcu cos
        q = torch.mean(q, dim=1)
    if len(r.shape) == 3:  # reference.shape is (T, h*w, D), we must pool h*w before calcu cos
...
class VideoSimMapModel(object):
    ...
    """
    def __init__(self, concurrency=4, device='cuda:0'):
    ...
    def forward(self,
        ...
        if use_cuda:
            func = partial(sim_map_gpu, normalize_input=normalize_input, similarity_type=similarity_type, device=self.device)
        else:
...
def dtw(sim_matrix: np.ndarray, discontinue=3, min_sim=0.2, min_length=5, **kwargs) -> List[List[int]]:
    """
