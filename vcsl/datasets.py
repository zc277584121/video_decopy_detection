...
"""
import os
from typing import Sequence, Callable, Tuple, Any, Dict, List
...
class PairDataset(Dataset):
    def __init__(self,
        ...
        if os.path.isdir(self.root):
            self.trans_key_func = lambda x: x + ".npy"
        elif self.root.endswith('hdf5'):
            self.trans_key_func = lambda x: x
        self.reader = build_reader(store_type, data_type, **kwargs)
    ...
    def __getitem__(self, item) -> Tuple[str, str, Any, Any]:
        ...
        if os.path.isdir(self.root):
            ...
            query_np = self.reader.read(query_path)
            gallery_np = self.reader.read(gallery_path)

        elif self.root.endswith('hdf5'):
            feature_file = h5py.File(self.root, "r")
            query_np = feature_file[query_file][:]
            gallery_np = feature_file[gallery_file][:]
        return query_id, gallery_id, query_np, gallery_np
